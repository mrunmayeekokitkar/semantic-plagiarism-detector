...
from src.db.incidents import get_all_incidents, DEFAULT_DB_PATH
...
def get_incidents_last_24h(db_path: str = DEFAULT_DB_PATH) -> List[Dict[str, Any]]:
    ...
    """
    cutoff_time = (datetime.now(timezone.utc) - timedelta(hours=24)).replace(microsecond=0).isoformat()
    
    all_incidents = get_all_incidents(db_path)
    recent_incidents = [
        inc for inc in all_incidents 
        if inc.get('date_flagged', '') >= cutoff_time
    ]
...
def send_email(to_emails: List[str], subject: str, html_body: str) -> bool:
    ...
    if not all([smtp_server, smtp_username, smtp_password]):
        logger.error("SMTP configuration incomplete. Please set SMTP_SERVER, SMTP_USERNAME, and SMTP_PASSWORD.")
        return False
    ...
    try:
        ...
        logger.info(f"Daily summary email sent successfully to {len(to_emails)} recipients")
        return True
